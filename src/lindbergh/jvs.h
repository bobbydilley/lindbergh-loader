...
#define STATUS_CHECKSUM_FAILURE                                                \
  0x03 // the checksum on the command packet did not match a computed checksum
#define STATUS_OVERFLOW                                                        \
  0x04 // an overflow occurred while processing the command

/* Reporting for each individual command */
#define REPORT_SUCCESS 0x01 // all went well
#define REPORT_PARAMETER_ERROR1                                                \
  0x02 // TODO: work out difference between this one and the next
#define REPORT_PARAMETER_ERROR2 0x03
#define REPORT_BUSY                                                            \
  0x04 // some attached hardware was busy, causing the request to fail
...
#define CMD_SET_COMMS_MODE                                                     \
  0xF2 // switch communications mode for devices that support it, for
       // compatibility
#define CMD_REQUEST_ID 0x10 // requests an ID string from a device
#define CMD_COMMAND_VERSION                                                    \
  0x11 // gets command format version as two BCD digits packed in a byte
#define CMD_JVS_VERSION                                                        \
  0x12 // gets JVS version as two BCD digits packed in a byte
#define CMD_COMMS_VERSION                                                      \
  0x13 // gets communications version as two BCD digits packed in a byte
#define CMD_CAPABILITIES                                                       \
  0x14 // gets a special capability structure from the device
...
#define ENCODINGS                                                              \
  [                                                                            \
    "unknown", "ascii numeric", "ascii alphanumeric", "alphanumeric/katakana", \
    "alphanumeric/SHIFT-JIS"                                                   \
  ]
...
extern unsigned char outputBuffer[JVS_MAX_PACKET_SIZE],
