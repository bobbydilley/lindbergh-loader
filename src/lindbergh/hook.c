...
/**
 * Signal handler for the SIGSEGV signal, which is triggered when a process
 * tries to access an illegal memory location.
 * @param signal
 * @param info
 * @param ptr
 */
...
FILE *fopen(const char *restrict pathname, const char *restrict mode) {
  FILE *(*_fopen)(const char *restrict pathname, const char *restrict mode) =
  ...
  if ((strcmp(pathname, "/usr/lib/boot/logo.tga") == 0) ||
      (strcmp(pathname, "/usr/lib/boot/logo.tga") == 0)) {
...
FILE *fopen64(const char *pathname, const char *mode) {
  FILE *(*_fopen64)(const char *restrict pathname, const char *restrict mode) =
...
size_t fread(void *buf, size_t size, size_t count, FILE *stream) {
  size_t (*_fread)(void *buf, size_t size, size_t count, FILE *stream) =
...
int ioctl(int fd, unsigned int request, void *data) {
  ...
  if ((request == SIOCSIFADDR) || (request == SIOCSIFFLAGS) ||
      (request == SIOCSIFHWADDR) || (request == SIOCSIFHWBROADCAST) ||
      (request == SIOCDELRT) || (request == SIOCADDRT) ||
...
int select(int nfds, fd_set *restrict readfds, fd_set *restrict writefds,
  ...
  int (*_select)(int nfds, fd_set *restrict readfds, fd_set *restrict writefds,
                 fd_set *restrict exceptfds, struct timeval *restrict timeout) =
...
int system(const char *command) {
  ...
  if (strcmp(command, "lspci | grep \"Multimedia audio controller: %Creative\" "
                      "> /dev/null") == 0)
    return 0;
 ...
 */
void kswap_collect(void *p) { return; }
...
/**
 * Callback function to get the offset and size of the execution program in
 * memory of the ELF we hook to.
 */
static int callback(struct dl_phdr_info *info, size_t size, void *data) {
  if ((info->dlpi_phnum >= 3) && (info->dlpi_phdr[2].p_type == PT_LOAD) &&
      (info->dlpi_phdr[2].p_flags == 5)) {
    elf_crc = get_crc32(
        (void *)(info->dlpi_addr + info->dlpi_phdr[2].p_vaddr + 10), 128);
  }
  return 1;
}
...
void getCPUID() {
  ...
  sprintf(cpu_vendor.cpuid, "%.4s%.4s%.4s", (const char *)&cpu_vendor.ebx,
          (const char *)&cpu_vendor.edx, (const char *)&cpu_vendor.ecx);
  if (getConfig()->showDebugMessages) {
...
char *getenv(const char *name) {
  ...
  if ((strcmp(name, "TEA_DIR") == 0) &&
      ((getConfig()->crc32 == VIRTUA_TENNIS_3) ||
       (getConfig()->crc32 == VIRTUA_TENNIS_3_TEST) ||
