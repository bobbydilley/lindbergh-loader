...
typedef enum { STATIC, NETWORK_TYPE, ETH0, ETH1, CREDIT, BACKUP } eepromSection;
...
void addCRCtoBuffer(int section) {
  ...
  memcpy(&eepromBuffer[eepromOffsetTable[section].offset], &crc,
         sizeof(uint32_t));
}
...
int getRegion() { return eepromBuffer[14]; }
...
int eepromSettingsInit(FILE *eeprom) {
  ...
  if (size >= 832) // eeprom initialized at least by SEGABOOT
  {
    ...
    if ((createStaticSection() != 0) || (createNetworkTypeSection() != 0) ||
        (createEthSection(ETH0) != 0) || (createEthSection(ETH1) != 0) ||
    ...
    addCRCtoBuffer(CREDIT);
    if ((writeSectiontoFile(eeprom, STATIC) != 0) ||
        (writeSectiontoFile(eeprom, NETWORK_TYPE) != 0) ||
        (writeSectiontoFile(eeprom, ETH0) != 0) ||
        (writeSectiontoFile(eeprom, ETH1) != 0) ||
