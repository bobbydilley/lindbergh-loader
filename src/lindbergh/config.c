...
char *getGameName() { return config.gameTitle; }

char *getDVPName() { return config.gameDVP; }

char *getGameID() { return config.gameID; }
...
int readConfig(FILE *configFile, EmulatorConfig *config) {
  ...
  while (fgets(buffer, MAX_LINE_LENGTH, configFile)) {
    ...
    if (strcmp(command, "WIDTH") == 0)
      ...
      config->keymap.testKey = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "SERVICE_KEY") == 0)
      config->keymap.serviceKey = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "PLAYER1_COIN_KEY") == 0)
      config->keymap.p1CoinKey = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "PLAYER2_COIN_KEY") == 0)
      config->keymap.p2CoinKey = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "PLAYER1_START_KEY") == 0)
      config->keymap.p1StartKey = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "PLAYER2_START_KEY") == 0)
      config->keymap.p2StartKey = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "BUTTON1") == 0)
      config->keymap.button1 = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "BUTTON2") == 0)
      config->keymap.button2 = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "BUTTON3") == 0)
      config->keymap.button3 = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "BUTTON4") == 0)
      config->keymap.button4 = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "BUTTON_UP") == 0)
      config->keymap.buttonUp = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "BUTTON_DOWN") == 0)
      config->keymap.buttonDown = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "BUTTON_LEFT") == 0)
      config->keymap.buttonLeft = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "BUTTON_RIGHT") == 0)
      config->keymap.buttonRight = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "ANALOGUE_UP") == 0)
      config->keymap.analogueUp = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "ANALOGUE_DOWN") == 0)
      config->keymap.analogueDown = atoi(getNextToken(NULL, " ", &saveptr));
    else if (strcmp(command, "ANALOGUE_LEFT") == 0)
...
int initConfig() {
  ...
  if (detectGame(config.crc32) != 0) {
    printf("Warning: Unsure what game with CRC 0x%X is. Please submit this new "
           ...
           "https://github.com/bobbydilley/lindbergh-loader/issues/"
           "new?title=Please+add+new+game+0x%X&body=I+tried+to+launch+the+"
           "following+game:\n",
           config.crc32, config.crc32);
  }
  // Button: 1Keymap: T 842084408 - R 1479684656 - G 2
  // Bugs
  switch (config.gameType) {
  case DRIVING:
    config.keymap.testKey = 28;
    config.keymap.serviceKey = 39;
    config.keymap.p1CoinKey = 14;
    config.keymap.p2CoinKey = 15;
    config.keymap.analogueUp = 111;
    config.keymap.analogueDown = 116;
    config.keymap.analogueLeft = 113;
    config.keymap.analogueRight = 114;
    config.keymap.p1StartKey = 10;
    // Unsupported p2
    config.keymap.p2StartKey = 0;
    config.keymap.button1 = 24;
    config.keymap.button2 = 25;
    config.keymap.button3 = 26;
    config.keymap.button4 = 27;
    config.keymap.buttonUp = 29;
    config.keymap.buttonDown = 30;
    config.keymap.buttonLeft = 31;
    config.keymap.buttonRight = 32;
    break;
  case SHOOTING:
  case FIGHTING:
  default:
    ...
    config.keymap.p1CoinKey = 14;
    config.keymap.p2CoinKey = 15;
    config.keymap.buttonUp = 111;
    config.keymap.buttonDown = 116;
    config.keymap.buttonLeft = 113;
    config.keymap.buttonRight = 114;
    config.keymap.p1StartKey = 10;
    // Unsupported p2
    config.keymap.p2StartKey = 0;
    config.keymap.button1 = 24;
